...
export const TripPlannerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [items, setItems] = useState<PlanItem[]>(() => {
    if (typeof window !== 'undefined') {
      ...
      const saved = safeStorage.getItem<PlanItem[]>('trip_planner_items', []);
  ...
  const [travelers, setTravelers] = useState<number>(() => {
    if (typeof window !== 'undefined') {
      ...
      return safeStorage.getItem<number>('trip_planner_travelers', 2) || 2;
    }
    return 2;
  });
  ...
  const [durationDays, setDurationDays] = useState<number>(() => {
    if (typeof window !== 'undefined') {
      ...
      return safeStorage.getItem<number>('trip_planner_days', 8) || 8;
    }
    return 8;
  });
  ...
  useEffect(() => {
    safeStorage.setItem('trip_planner_items', items);
  }, [items]);

  useEffect(() => {
    safeStorage.setItem('trip_planner_travelers', travelers);
    safeStorage.setItem('trip_planner_days', durationDays);
