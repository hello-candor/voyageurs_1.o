...
export const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose, onNavigate }) => {
    ...
    return (
        <div className="fixed inset-0 z-[300] bg-med-blue/95 dark:bg-black/95 backdrop-blur-xl animate-in fade-in duration-300 flex flex-col">
            <div className="max-w-3xl mx-auto w-full p-6 md:p-12 flex flex-col h-full">
                ...
                <div className="flex flex-col items-center mb-10 text-center">
                    <h2 className="font-serif text-3xl md:text-4xl text-white mb-4">Ask Céleste</h2>
                ...
                <form onSubmit={handleAiSearch} className="relative w-full mb-10">
                    ...
                    <input 
                        ...
                        className="w-full bg-white/10 border border-white/10 rounded-[2rem] pl-16 pr-6 py-6 text-xl md:text-2xl text-white placeholder:text-white/20 outline-none focus:bg-white/20 focus:border-white/30 transition-all font-serif"
                    />
                    <button 
                        type="submit"
                        disabled={!query.trim() || isAiLoading}
                        className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-med-terracotta hover:bg-[#c56143] text-white rounded-full transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    {aiResponse ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 opacity-50">
                            {[
                                ...
                                >
                                    <suggestion.icon size={20} className="text-white/60 group-hover:text-med-terracotta transition-colors" />
                                    <span className="text-white/80 font-medium">{suggestion.text}</span>
                                </button>
                            ))}
                        </div>
                    )}
